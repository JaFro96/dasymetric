...
tmap::tmap_mode("plot")
map_lu = tmap::tm_shape(population_counts) + 
  tmap::tm_borders() + 
...
data("buildings")
map_bu = tmap::tm_shape(population_counts) + 
  tmap::tm_borders() + 
  tmap::tm_shape(buildings) + 
  tmap::tm_polygons("funktion") + 
  tmap::tm_layout(frame.lwd = 3, legend.position = c("right", "top"))
tmap_arrange(map_lu, map_bu, ncol = 1)
```
...
map_dm_lu = tmap::tm_shape(dm_pop) + tmap::tm_polygons("pop_prediction", title = "DM-interpolation (landuse)")
map_dm_bu = tmap::tm_shape(dm_pop_buildings) + tmap::tm_polygons("pop_prediction", title = "DM-interpolation (buildings)")
tmap_arrange(map_dm_bu, map_dm_lu,map_census, ncol = 2)
```
...
cat("Absolute error of dasymetric mapping using landuse data: ",sum(errors$Prediction_Inaccuracy_Dasymetric_Mapping))
...
err2 = tmap::tm_shape(errors) + tmap::tm_polygons("Prediction_Inaccuracy_Dasymetric_Mapping", title = "Absolute Errors (DM landuse)")
